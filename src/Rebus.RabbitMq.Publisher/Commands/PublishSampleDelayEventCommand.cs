...
namespace Rebus.RabbitMq.Publisher.Commands
{
    public class PublishSampleDelayEventCommand : Command
    {
        ...
        public PublishSampleDelayEventCommand(IBus bus, ILogger<PublishSampleDelayEventCommand> logger) : base("publish-sample-delay-event", "Publishes a SampleDelayEvent to RabbitMQ")
        {
            _bus = bus;
            _logger = logger;
